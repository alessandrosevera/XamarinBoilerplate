...
{
    public partial class AgencyPage : ContentPage, Presenter, OverlayHost, LocalizableElement, TabItemPage
    {
        ...
        public AppTabs AppTab => AppTabs.Agency;
        ...
        {
            if (PageContent is Phone.AgencyPageContent phonePageContent)
            {
        ...
        private void ActuatePageContent()
        {
            switch (DeviceInfo.Idiom.ToString())
            {
                case nameof(DeviceIdiom.Tablet):
                    // @AS TABLET
                    PageContent = new Phone.AgencyPageContent();
                    break;
